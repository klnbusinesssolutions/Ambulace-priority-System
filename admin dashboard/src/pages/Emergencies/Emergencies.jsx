...
import { normalizeEmergencyStatus } from "../../utils/emergencyLifecycle.js";
    ...
    () =>
      emergencies.filter(
        (item) =>
          ...
          (status === "All statuses" || normalizeEmergencyStatus(item.status) === normalizeEmergencyStatus(status)) &&
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 sm:flex-row">
        ...
        <Select className="sm:w-48" value={status} onChange={(event) => setStatus(event.target.value)} options={["All statuses", "reported", "assigned", "en_route", "arrived", "completed"]} />
      </div>
      <EmergencyCards
        rows={rows}
