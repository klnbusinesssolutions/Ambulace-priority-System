...
export default function Input({ className, label, helperText, error, ...props }) {
  const control = (
    <input
      className={cn(
        "focus-ring h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-400 transition-colors shadow-xs disabled:opacity-50 disabled:cursor-not-allowed",
