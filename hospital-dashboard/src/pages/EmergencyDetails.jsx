...
import { useEmergencies, useEmergency } from '../hooks/useEmergencies';
...
function EmergencyDetails() {
  ...
  const { emergencies: allEmergencies } = useEmergencies();
  ...
  const emergencyIdMap = useMemo(() => {
    const sortedAsc = [...allEmergencies].sort(
  ...
  const displayId = (emergency && emergencyIdMap.get(emergency.id)) || emergency?.id;
